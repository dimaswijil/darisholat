...
def replacer(match):
    ...
    if func_name in translations:
        cases = ""
        for lang, trans in translations[func_name].items():
            cases += f'        case "{lang}": return "{trans}"\n'
        return match.group(0) + cases
    return match.group(0)

new_content = re.sub(r'(static func (\w+).*?\{\s*switch lang \{\s*case "ar": return ".*?"\s*case "id": return ".*?"\n)', replacer, content)

with open("DariSholat/Localization.swift", "w") as f:
    f.write(new_content)

print("Updated Localization.swift")
